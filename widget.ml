...
let list_layout (pair: widget -> widget -> widget)
  ...
  let spacer : widget = space (0,0) in
  List.fold_right pair ws spacer
...
let make_controller (v: 'a) : 'a value_controller =
  let r = {contents = v} in
  let l = {contents = []} in
  let rec calls (v2:'a) (list: ('a -> unit) list) : unit =
    begin match list with
      |[] -> ()
      |head::tail -> head v2; calls v2 tail
    end in
  {add_change_listener = (fun f -> l.contents <- l.contents@[f]);
   get_value = (fun () -> r.contents);
   change_value = (fun v2 -> r.contents <- v2; calls v2 l.contents)
  }
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  ...
  let h = snd (lab.size ()) in
  let status_square = 
    {repaint = (fun (g: Gctx.gctx) ->
      if vc.get_value ()
      then Gctx.fill_rect (Gctx.with_color g Gctx.green) (0,0) (h,h)
      else Gctx.fill_rect (Gctx.with_color g Gctx.red) (0,0) (h,h));
      handle = (fun _ _ -> ());
      size = (fun () -> (h,h))
    } in
  let final = hlist [status_square; space (5, h); lab] in
  { repaint = final.repaint;
    handle = (fun (g: Gctx.gctx) (e: Gctx.event) ->
      if Gctx.event_type e = Gctx.MouseDown && vc.get_value () then 
        vc.change_value false 
      else if Gctx.event_type e = Gctx.MouseDown then vc.change_value true);
    size = final.size
  }, vc
...
(** Creates a slider widget with given length and max value. *)
let slider (length: int) (max: int) : widget * int value_controller =
  let vc = (make_controller max) in
  {repaint = (fun (g: Gctx.gctx) ->
    let tmax = ((length * max)/max) in
    let tvalue = ((length * vc.get_value())/max) in
    (Gctx.fill_rect (Gctx.with_color g Gctx.red) (0, 0) (tmax, 3);
     Gctx.fill_rect (Gctx.with_color g Gctx.green) (0, 0) (tvalue, 3)));
   handle = (fun (g: Gctx.gctx) (e: Gctx.event) ->
     let (mouse_x, mouse_y) = Gctx.event_pos e g in
     if Gctx.event_type e = Gctx.MouseDrag then 
       vc.change_value ((mouse_x * max)/length + 1)
     else if Gctx.event_type e = Gctx.MouseDown then 
       vc.change_value ((mouse_x * max)/length + 1));
   size = fun () -> (length, 10)
  }, vc
